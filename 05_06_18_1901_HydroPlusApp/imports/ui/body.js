...
var water_stations_guest = [['Rio De Flag at Aspen Valley (south)', 35.198505, -111.594887,
                            '<h3>Rio De Flag at Aspen Valley (south)</h3> <br> <a href="/gaugeone"> Submit Data for this Location </a> <br> <a href="/gaugeonesubmissionguest">View Submitted Data</a>'],
                            ['Rio de Flag at Aspen Valley (north)', 35.210533, -111.587653,
                            '<h3>Rio De Flag at Aspen Valley (north)</h3> <br> <a href="/gaugetwo"> Submit Data for this Location </a> <br> <a href="/gaugetwosubmissionguest">View Submitted Data</a>'],
                            ["Siccs Gauge", 35.18620, -111.65840,
                            '<h3>School of Informatics and Cyber Systems(SICCS)</h3> <br> <a href="/gaugethree"> Submit Data for this Location </a> <br> <a href="/gaugethreesubmissionguest">View Submitted Data</a>']];
var geoMarker;
...
Template.GuestNavOne.events({
  ...
  'submit .submissionsGuest1'(event){
    event.preventDefault();
    Router.go('gaugeonesubmissionguest');
  },
});
...
Template.GuestNavTwo.events({
  ...
  'submit .submissionsGuest2'(event){
    event.preventDefault();
    Router.go('gaugetwosubmissionguest');
  },
});
...
Template.GuestNavThree.events({
  ...
  'submit .submissionsGuest3'(event){
    event.preventDefault();
    Router.go('gaugethreesubmissionguest');
  },
});
...
Template.gaugeonesubmissionguest.helpers({
    submitted_data() {
        // Show newest tasks at the top
        return Submitted_Data.find({}, { sort: { createdAt: -1 } });
    },
...
Template.gaugetwosubmissionguest.helpers({
    submitted_data() {
        // Show newest tasks at the top
        return Submitted_Data.find({}, { sort: { createdAt: -1 } });
    },
...
Template.gaugethreesubmissionguest.helpers({
    submitted_data() {
        // Show newest tasks at the top
        return Submitted_Data.find({}, { sort: { createdAt: -1 } });
    },
...
Template.gaugethreesubmissionguest.events({
  'submit .gaugeThreeSubmit'(event){
    event.preventDefault();
    Router.go('gaugethree');
  },
...
Template.gaugeonesubmissionguest.onRendered(function() {
  var testVal = Submitted_Data.find({gaugename: "RioDeFlagAspenValleySouth"});
  var val = testVal.fetch();
  var retVal = 0;
  var retVals = [];
...
Template.gaugetwosubmissionguest.onRendered(function() {
  var testVal = Submitted_Data.find({gaugename: "RioDeFlagAspenValleyNorth"});
  var val = testVal.fetch();
  var retVal = 0;
  var retVals = [];
...
Template.gaugethreesubmissionguest.onRendered(function() {
  var testVal = Submitted_Data.find({gaugename: "SICCS"});
  var val = testVal.fetch();
  var retVal = 0;
  var retVals = [];
...
// When created, we need to display the map for guest.
Template.guestmap.onRendered(function() {
  var self = this;
  ...
  GoogleMaps.ready('map', function(map) {
    ...
    self.autorun(function() {
      var geolocationCoord = Geolocation.latLng();
      if (! geolocationCoord){
        for (var i = 0; i < water_stations_guest.length; i++) {
          var water_gauge = water_stations_guest[i];
          var marker = new google.maps.Marker({
            position: {lat: water_gauge[1], lng: water_gauge[2]},
            map: map.instance,
            draggable: false
          ...
          google.maps.event.addListener(marker, 'click', (function(marker, i) {
            return function() {
              stationiInfoWindow.setContent(water_stations_guest[i][3]);
              stationiInfoWindow.open(map, marker);
            }
          })(marker, i));
        }
