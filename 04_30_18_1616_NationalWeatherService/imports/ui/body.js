...
Template.gaugeonesubmission.onRendered(function() {
  ...
  //var userTestVal = meteorUserName = Meteor.user().emails[0].address
  var userTestVal = Submitted_Data.find({meteorUserName: Meteor.user().emails[0].address});
...
Template.gaugeone.events({
    'submit .gaugeOneSubmitGoHome'(event){
      event.preventDefault();
      Router.go('Home');
    },
...
Template.gaugetwo.events({
  'submit .gaugeTwoSubmitGoHome'(event){
    event.preventDefault();
    Router.go('Home');
  },
...
Template.map.onRendered(function() {
  var self = this;
  // If this is a Guest User, they will not receive notifications
  if(Meteor.userId() == null){
    console.log("Not logged in");
   }

   // If this is a Login User, they will receive notifications
   // Calls the Open Weather API from the server
   else{
